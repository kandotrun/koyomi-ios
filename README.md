...
- ピンした予定を開始12時間前からLive Activityへ自動表示。iOSの同時表示上限内で開始が近い予定を優先
- Live Activityは予定開始で0秒に停止し、アプリの次回同期時に終了。Appleの標準Live Activity仕様によりDynamic Islandは最大8時間、その後もロック画面には最大4時間残る場合があります
...
- バックエンド、ログイン、分析SDK、アプリ独自の外部通信はありません。
- カレンダー内容を外部サーバーへ送信しません。ピン予定のLive ActivityはAppleのOS機能により、ユーザー自身のペアリング済みApple WatchやMacにも表示される場合があります。転送は開発者側では停止できず、各端末のLive Activities設定でユーザーが無効化できます。
